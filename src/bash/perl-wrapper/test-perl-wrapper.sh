#!/bin/bash 
# file: src/bash/perl-wrapper/test-perl-wrapper.sh
      ...
      doLogTestRunEntry 'START'
		while read -r test_file ; do (
            ...
            then
               ...
               $function_name
               doLog "INFO test-perl-wrapper loop exit_code: $exit_code" 
               doLogTestRunEntry 'INFO' ' '$(date "+%H:%M:%S")
		...
		done < <(find src/bash/perl-wrapper/tests -type f -name '*.sh')

	);
	done < <(cat $product_instance_dir/src/bash/perl-wrapper/tests/run-perl-wrapper-tests.lst)
...
doLog(){
   ...
   test -t 1 && echo " [$type_of_msg] `date "+%Y.%m.%d-%H:%M:%S"` [perl-wrapper][@$host_name] [$$] $msg "
...
doSetVars(){
   ...
   . "$product_instance_dir/src/bash/perl-wrapper/funcs/log-test-run-entry.func.sh"
...
#----------------------------------------------------------
# Purpose:
# a simplistic tester for the simplistic bash stub perl-wrapperer
#----------------------------------------------------------
#
#----------------------------------------------------------
...
#----------------------------------------------------------
#
# eof file: src/bash/perl-wrapper/test-perl-wrapper.sh
