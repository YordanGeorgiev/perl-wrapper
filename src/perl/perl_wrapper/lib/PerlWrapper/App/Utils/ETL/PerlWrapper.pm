package PerlWrapper::App::Utils::ETL::PerlWrapper ; 
...
=head1 SYNOPSIS
	...
	doResolves the product version and base dirs , bootstraps cnfig files if needed
		...
		my $objPerlWrapper = 
			'PerlWrapper::App::Utils::ETL::PerlWrapper'->new ( \$appConfig ) ; 
=cut 
...
=cut
	...
	sub get {
		...
		my $self = shift;
		my $name = shift;
		croak "\@PerlWrapper.pm sub get TRYING to get undefined name" unless $name ;  
		croak "\@PerlWrapper.pm sub get TRYING to get undefined value" unless ( $self->{"$name"} ) ; 
